...
class UserModel
{
    ...
    public function insertar($nombre, $apellido, $email, $password)
    {
        ...
        try {
            $stament = $this->connection->prepare("INSERT INTO usuarios (nombre, apellido, email, password) VALUES (:nombre, :apellido, :email, :password)");
    ...
    {
        $query = "SELECT COUNT(*) FROM alquileres WHERE id = :id";
        $stmt = $this->connection->prepare($query);
        $stmt->bindParam(':id', $id_alquiler);
        $stmt->execute();
        $count = $stmt->fetchColumn();
